...
* Her velger man hvilke `komponenter` man ønsker å gjøre `tilgjengelig for sin organisasjon`.
* Hvilken `organisasjon` man har valgt vises i `øvre høyre hjørne` (I eksemplene vil det være Møre og Romsdal).
* De som har et `grønt symbol` med et pluss i, er komponenter som `kan legges til`.
* De med `rødt symbol` med en minus i, er komponenter som allerede `er lagt til`.
* Man klikker på symbolet for å legge til eller fra komponenten.
* Hvis man klikker på `tannhjulet` bak det røde eller grønne symbolet, vil man få opp et vindu som viser `informasjon om komponenten`.
